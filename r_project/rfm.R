...
for (i in 1:clusters) {
	...
	logintimes_d30 <- append(logintimes_d30, round(mean(c$logintimes_d30), digits=2))
	duration_d30 <- append(duration_d30, round(mean(c$duration_d30), digits=2))
	total <- append(total, mytable[i])
	print(paste("kcuster ", i, " size ", nrow(c)))
	print(summary(c))}
out <- data.frame(paytimes_d30, payamount_d30, date_diff, logintimes_d30, duration_d30, total)
print(out)

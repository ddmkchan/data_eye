...
def get_iqiyi_comments(qitanid, page, page_size, sort, tvid):
	url = "http://api.t.iqiyi.com/qx_api/comment/get_video_comments?aid=%s&categoryid=6&cb=fnsucc&escape=true&need_reply=true&need_total=1&page=%s&page_size=%s&page_size_reply=3&qitan_comment_type=1&qitancallback=fnsucc&sort=%s&qitanid=%s&tvid=%s" % (qitanid, page, page_size, sort, qitanid, tvid)
	print url
	r = s.get(url)
...
if __name__ == '__main__':
	...
	get_iqiyi_comments(10920960, 3, 10, IQIYI_SORT_TYPE[1],364737400)
